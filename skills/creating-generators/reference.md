...
When completing `generator-workbench` integration, generate `__tests__/workbench-integration.test.ts` in the developer's project using the template below. Replace import paths with the project's actual runtime and config factories. Remove blocks marked with `// ─── workbench-template only` for `workbench-basic` or `workbench-cloud` profiles. Remove blocks marked `// ─── workbench-cloud only` for `workbench-basic` profiles.

Do **not** import from `@atomm-developer/generator-workbench/testing`. This file must be entirely self-contained and use only Vitest APIs.

```ts
// __tests__/workbench-integration.test.ts
// Auto-generated by creating-generators skill.
// Profile: workbench-template  ← replace with actual profile: workbench-basic | workbench-cloud | workbench-template
// Replace the import paths below with actual paths from this project.

import { describe, it, expect, beforeEach } from 'vitest'
import { createExampleRuntime } from '../src/runtime'           // ← replace with actual runtime factory path
import { getWorkbenchConfig } from '../src/workbench-config'   // ← replace with actual config factory path

// ─── Runtime Contract: interface ────────────────────────────────────────────

describe('Runtime Contract: interface', () => {
  it('has mount / getState / setState / getPanelSchema', () => {
    ...
    expect(typeof runtime.getPanelSchema).toBe('function')
  })

  it('mount() returns object with unmount()', async () => {
    const container = document.createElement('div')
    const result = await createExampleRuntime().mount({ mode: 'full', container })
    expect(typeof result.unmount).toBe('function')
  })

  it('getState() returns JSON-serializable object', () => {
    const state = createExampleRuntime().getState()
    expect(() => JSON.stringify(state)).not.toThrow()
  })

  it('getPanelSchema() returns valid PanelSchema', () => {
    const schema = createExampleRuntime().getPanelSchema()
    expect(schema).toHaveProperty('generatorId')
    expect(Array.isArray(schema.groups)).toBe(true)
  })

  it('export is NOT a method on the runtime object (registered via sdk.export.register())', () => {
    const runtime = createExampleRuntime()
    expect((runtime as Record<string, unknown>)['export']).toBeUndefined()
  })
})

// ─── Runtime Contract: events ────────────────────────────────────────────────

describe('Runtime Contract: events', () => {
  it('subscribe: emits ready after mount', async () => {
    const runtime = createExampleRuntime()
    const events: string[] = []
    runtime.subscribe?.((e) => events.push(e.type))
    await runtime.mount({ mode: 'full', container: document.createElement('div') })
    expect(events).toContain('ready')
  })

  it('subscribe: emits state-change after patchState', async () => {
    const runtime = createExampleRuntime()
    const events: Array<{ type: string }> = []
    runtime.subscribe?.((e) => events.push(e))
    await runtime.mount({ mode: 'full', container: document.createElement('div') })
    await runtime.patchState?.({ 'params.width': 100 }, { source: 'test' })
    expect(events.some((e) => e.type === 'state-change')).toBe(true)
  })

  // ─── workbench-template only ─ remove this block for workbench-basic / workbench-cloud ──

  it('params_change event contains cover field (required for template publish)', async () => {
    const runtime = createExampleRuntime()
    const paramsChanges: Array<{ type: string; data?: { cover?: unknown } }> = []
    runtime.subscribe?.((e) => {
      if (e.type === 'params_change') paramsChanges.push(e as never)
    })
    await runtime.mount({ mode: 'full', container: document.createElement('div') })
    await runtime.patchState?.({ 'params.width': 120 }, { source: 'panel' })
    // cover must be a non-empty string; undefined means "Publish as Template" dialog will lack cover
    const hasCover = paramsChanges.some(
      (e) => typeof e.data?.cover === 'string' && e.data.cover.length > 0,
    )
    expect(hasCover).toBe(true)
  })

  // ─── end workbench-template only ─────────────────────────────────────────
})

// ─── Workbench Config: pre-flight ────────────────────────────────────────────

describe('Workbench Config: pre-flight', () => {
  it('atommProEnv is defined', () => {
    expect(getWorkbenchConfig().atommProEnv).toBeDefined()
  })

  it('atommProDomain is defined', () => {
    expect(getWorkbenchConfig().atommProDomain).toBeDefined()
  })

  // ─── workbench-template only ─ remove this block for workbench-basic / workbench-cloud ──

  it('templateEnabled is true', () => {
    expect(getWorkbenchConfig().templateEnabled).toBe(true)
  })

  // ─── end workbench-template only ─────────────────────────────────────────

  // ─── workbench-cloud only ─ remove this block for workbench-basic ────────

  it('getCloudSaveOptions returns { title, snapshot }', () => {
    const ctx = {
      state: { meta: { generatorId: 'x' }, params: {} },
      runtime: {} as never,
      config: {} as never,
    }
    const result = getWorkbenchConfig().getCloudSaveOptions?.(ctx)
    expect(result).toHaveProperty('title')
    expect(result).toHaveProperty('snapshot')
    expect(() => JSON.stringify(result?.snapshot)).not.toThrow()
  })

  // ─── end workbench-cloud only ────────────────────────────────────────────
})

// ─── Global exposure ──────────────────────────────────────────────────────────

describe('Global runtime exposure', () => {
  it('window.__GENERATOR_RUNTIME__ is assigned after init', () => {
    // If the entry script assigns window.__GENERATOR_RUNTIME__ = runtime,
    // this test confirms the convention is followed.
    // Replace with the actual init call if it can be imported here.
    expect(typeof (window as Record<string, unknown>)['__GENERATOR_RUNTIME__']).not.toBe('undefined')
  })
})
```
